...
def info_popup(event=None):
    ...
    top.title("About: Title Normalizer v1.4")
    ...
    \n✅ Dasturdan fayl nomidagi probellar oʻrniga (_) qoʻyishda ham foydalanishingiz mumkin. \n\
    ...
    \n • Fayl nomidagi probellarni yoʻqotib, oʻrniga tagchiziq qoʻyish uchun «Reverse» tugmasini bosing.\
...
f3 = Button()
oyna.bind('<F3>', ocean_bg)

f4 = Button()
oyna.bind('<F4>', black_bg)
...
def rev_btn(event=None):
    rev.configure(bg="#d72631", fg="#fcf5e5")
...
def grey_btn(event=None):
    ...
    rev.configure(bg="#f0f0f0", fg="black")

# buttons and their bindings

clr = Button(oyna, text="Clear", width=6, command=clear_entry)
clr.grid(row=0, column=1, sticky="nw", padx=(0,5), pady=(5,5))

clr.bind("<Enter>", clear_btn)
clr.bind("<Leave>", grey_btn)

fch = Button(oyna, text="📂", command=get_file_name)
fch.grid(row=0, column=1, sticky="ne", padx=(0,5), pady=(5,5))

fch.bind("<Enter>", fch_btn)
fch.bind("<Leave>", grey_btn)

btn = Button(oyna, width=10, text="Normalize", command=underscore_remover)
btn.grid(row=0, column=1, sticky="sew", padx=(0,5), pady=(0,5))

btn.bind("<Enter>", norm_btn)
btn.bind("<Leave>", grey_btn)

rev = Button(oyna, text="Reverse", command=lambda:[add_underscore()])
rev.grid(row=1, column=1, sticky="new", padx=(0,5))

rev.bind("<Enter>", rev_btn)
rev.bind("<Leave>", grey_btn)

cpy = Button(oyna, text="Copy", command=lambda:[copy_to_clipboard(), clear_entry()])
